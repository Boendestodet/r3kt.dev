...
test('deducts generation cost from user balance', function () {
    ...
    expect($user->fresh()->balance)->toBe('9.9988');
    expect($user->fresh()->total_spent)->toBe('0.0013');
});
...
test('fails to deduct when insufficient balance', function () {
    ...
    expect($user->fresh()->balance)->toBe('0.0010');
    expect($user->fresh()->total_spent)->toBe('0.0000');
});
...
test('adds credits to user balance', function () {
    ...
    expect($user->fresh()->balance)->toBe('15.0000');
});
...
test('gets balance info correctly', function () {
    ...
    expect($balanceInfo['balance'])->toBe('10.0000');
    expect($balanceInfo['total_spent'])->toBe('2.5000');
